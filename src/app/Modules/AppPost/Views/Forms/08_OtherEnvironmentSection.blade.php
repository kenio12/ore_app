<div class="bg-white p-8 rounded-lg shadow mb-8">
            ...
            @foreach([
                ...
                <label class="flex items-center gap-2">
                    <input
                        ...
                        value="{{ $option }}"
                        {{ in_array($option, old('other_info.infrastructure', $post->other_info['infrastructure'] ?? [])) ? 'checked' : '' }}
            ...
            @foreach([
                ...
                <label class="flex items-center gap-2">
                    <input
                        ...
                        value="{{ $option }}"
                        {{ in_array($option, old('other_info.ci_cd', $post->other_info['ci_cd'] ?? [])) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    >
                    <span class="text-gray-700">{{ $option }}</span>
                </label>
            ...
            @foreach([
                ...
                <label class="flex items-center gap-2">
                    <input
                        ...
                        value="{{ $option }}"
                        {{ in_array($option, old('other_info.containers', $post->other_info['containers'] ?? [])) ? 'checked' : '' }}
                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    >
                    <span class="text-gray-700">{{ $option }}</span>
                </label>
    ...
    <div class="mb-6">
        ...
        >{{ old('other_info.tools', $post->other_info['tools'] ?? '') }}</textarea>
